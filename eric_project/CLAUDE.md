...
- `voice.chunkPlan` is **explicitly enabled** with `formatPlan.enabled: true` plus a regex replacement that strips `<EVAL:[^>]*>` from TTS. This is what keeps the model's diagnostic `<EVAL: ...>` classification tags out of the spoken output. Discovered the hard way: when `chunkPlan` is null/unset, the whole formatting pipeline is off — and the named default formatter `removeAngleBracketContent` doesn't run. The pipeline must be explicitly enabled. The regex replacement is belt-and-suspenders in case the named formatter misses a tag spanning a chunk boundary (chunking happens before formatting). The angle-bracket text is presumed preserved in `artifact.transcript` and `artifact.messages` — that's the documented behavior but not verified in writing; first test call should confirm by checking the saved transcript still contains `<EVAL:` tags.
