...
- `GET /transcript/:callId` — read-only proxy that fetches a Vapi call and returns a plain-text transcript view (BOT/PARTICIPANT speaker labels, header metadata, role-rolled-up body). Designed for Claude's `web_fetch` tool, which can't send auth headers. Optional gate via `?token=...` enforced only when `TRANSCRIPT_PROXY_TOKEN` env var is set.
- `GET /transcripts/recent?n=5` — same plain-text shape, concatenated across the N most recent calls (default 5, capped at 10). Same `?token=...` gate.
...
| `TRANSCRIPT_PROXY_TOKEN` | Optional. Shared secret required as `?token=...` query param on `/transcript/:callId` and `/transcripts/recent`. If unset, those endpoints are open (read-only). Use a long random string (e.g. `openssl rand -hex 32`). |
